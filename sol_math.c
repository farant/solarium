...
/* General 4x4 inverse by cofactor expansion (the classic adjugate/det form).
   Works on the flat column-major array; singular input degrades to identity
   rather than NaNs. P8 item 2 uses inverse(proj*view) to turn a depth sample
   back into a world position; SSAO (item 5) will reuse it. */
mat4 mat4_inverse(mat4 m) {
    ...
    if (det == 0.0f) return mat4_identity();   /* singular: degrade, don't NaN */
    det = 1.0f / det;
    for (i = 0; i < 16; i++) r.m[i] = inv[i] * det;
    return r;
}

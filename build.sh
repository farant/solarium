...
# Build + run the server auth layer test under the sanitizers. Tests users
# (PBKDF2), sessions (bearer tokens), and per-IP login throttle. Same flags
# and link model as srvdbtest.
if [ "$MODE" = "srvauthtest" ]; then
    vendor_objs
    clang -std=c11 -g -O1 -fno-omit-frame-pointer \
        -fsanitize=address,undefined \
        -Wall -Wextra -isystem vendor \
        srv_auth.c srv_db.c srv_rand.c sha256.c b64.c srv_auth_test.c vendor/sqlite3.o \
        -lpthread -lm \
        -o srv_auth_test
    echo "built ./srv_auth_test (ASan + UBSan) — run it; sanitizers report on stderr"
    exit 0
fi

# Build + run the server web layer test under the sanitizers. Tests string
# builder growth, HTML escape correctness, and page shape. Pure: no DB, no CivetWeb.
if [ "$MODE" = "srvwebtest" ]; then
    clang -std=c11 -g -O1 -fno-omit-frame-pointer \
        -fsanitize=address,undefined \
        -Wall -Wextra \
        srv_web.c srv_web_test.c \
        -o srv_web_test
    echo "built ./srv_web_test (ASan + UBSan) — run it; sanitizers report on stderr"
    exit 0
fi
